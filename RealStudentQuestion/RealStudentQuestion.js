...
async function loadExamConfig() {
    ...
    if (stored) {
        ...
        await loadSavedPassages(); // Load saved passages
        return;
    }
...
async function handlePassageSubmit() {
    ...
    try {
        ...
        if(passageData.passage_position){
            console.log('I am there i will not add')
            return;
        }
        
        showAlert('Passage added successfully!', 'success');
        passageaddedquestion = passageaddedquestion + passageData.passage_number;
        updateDisplay() 
        // Reset passage form
...
function updateDisplay() {
    ...
    if (questionsAddedDisplay) {
        questionsAddedDisplay.textContent = questions.length + passageaddedquestion;
    }
