...
function validateInputs() {
  ...
  if(Number(theoryMarksValue) > 90 || Number(theoryMarksValue < 10)){
...
async function handleCreateNewExam() {
  ...
  const newConfig = {
    theory_marks: await behaviourForMarkAwarded(),
...
async function behaviourForMarkAwarded(){
    ...
    const theoryAwarded = await getTheoryMarks( selectedClass.value, selectSubject.value);
    const theoryno = theoryAwarded || false;
    const objawardMarks  = theoryMarks.value;
    const objRealMrks = 100 - parseInt(theoryno.obj_award_mark);
    let savedValue = 0
    console.log(theoryAwarded);
    if(theoryno.obj_award_mark){
          if(objawardMarks !== objRealMrks){
   showDangerAlert(`The available mark is ${objRealMrks} and it has been updated to that score`);
